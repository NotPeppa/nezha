...
installation_check() {
    if docker compose version >/dev/null 2>&1; then
        ...
        if sudo $DOCKER_COMPOSE_COMMAND ls | grep -qw "$NZ_DASHBOARD_PATH/docker-compose.yaml" >/dev/null 2>&1; then
            NEZHA_IMAGES=$(sudo docker images --format "{{.Repository}}":"{{.Tag}}" | grep -w "notpeppa/nezha")
...
init() {
    ...
    if [ -n "$CUSTOM_MIRROR" ]; then
        GITHUB_RAW_URL="raw.githubusercontent.com/NotPeppa/nezha/main/script"
        GITHUB_URL=$CUSTOM_MIRROR
        Docker_IMG="ghcr.io\/notpeppa\/nezha"
    else
        if [ -z "$CN" ]; then
            GITHUB_RAW_URL="raw.githubusercontent.com/NotPeppa/nezha/main/script"
...
uninstall_dashboard_docker() {
    ...
    sudo docker rmi -f ghcr.io/notpeppa/nezha >/dev/null 2>&1
}
